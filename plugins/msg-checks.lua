...
---------------join req --------------
        ...
        --[[local res = 'https://api.telegram.org/bot567215035:AAGXTuI5rTqdVA31jufsjvldL0iCWtLmKU4/getChatMember?chat_id=@' .. group_channel .. '&user_id=' .. msg.from.id
		print(res)
		local coin = https.request(res)
		local data = json:decode(coin)
		local text = data.result.status
...
if not is_mod(msg) and not is_owner(msg) and not is_admin(msg) and not is_sudo(msg) and not is_whitelist(msg.from.id, msg.to.id) and msg.from.id ~= our_id and text == "left" or text == "kicked" then
		if lock_join_req == "yes" then
			...
			elseif not lang then
				tdcli.sendMessage(msg.to.id, msg.id, 0, '<b>User ID :</b> <code>'..msg.from.id..'</code>\n<b>Username :</b> '..('@'..msg.from.username or '<i>No Username</i>')..'\n<i>You Have Not Permission To Send Message, Please First Join in our channel!</i>' .. 'our channel: @' .. group_channel, 0, "html")
			end
		end
end]]--
---------------join req --------------
