...
local function ToOfan(msg, parts)
	if is_mod(msg) then
		local hash = "gp_lang:"..msg.to.id
		local lang = redis:get(hash)
		if parts[1]:lower() == 'unforce' or parts[1]:lower() == "حذف اجباری" then
			if parts[2]:lower() == 'add' or parts[2]:lower() == "اضافه" then
		...
		if parts[1]:lower() == 'force' or parts[1]:lower() == "اجباری" then
			if parts[2]:lower() == 'add' or parts[2]:lower() == "اضافه" then
				local add = redis:hget('addmeminv' ,msg.to.id)
				if not add then
		...
		if parts[1]:lower() == 'setadd' or parts[1]:lower() == "تنظیم اجباری" and parts[2] then
		...
		if parts[1]:lower() == 'getadd' or parts[1]:lower() == "دریافت اجباری" then
		...
		if parts[1]:lower() == 'addpm' or parts[1]:lower() == "پیام اجباری" then
