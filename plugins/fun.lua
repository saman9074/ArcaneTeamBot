...
--------------------------------
	if (matches[1]:lower() == 'jokinja' and not Clang) or (matches[1]:lower() == 'جوکاینجا' and Clang) or (matches[1]:lower() == 'جوک' and Clang) then
		local yon = http.request('https://fatemehalzahra.com/bot/chatbot/conversation_start.php?bot_id=1&say=جوک&convo_id=group_id' .. msg.to.id .. '&format=xml')	
		local handler = require("xmlhandler.tree")
		local parser = xml2lua.parser(handler)
		parser:parse(yon)		
		local text = handler.root.program_o.chat.line.response 
		str = ""
...
--------------------------------
	if (matches[1]:lower() == 'jazbmard' and not Clang) or (matches[1]:lower() == 'جذب مردان' and Clang) then
		local yon = http.request('https://fatemehalzahra.com/bot/chatbot/conversation_start.php?bot_id=1&say=شیما&convo_id=group_id' .. msg.to.id .. '&format=xml')	
		local handler = require("xmlhandler.tree")
		local parser = xml2lua.parser(handler)
		parser:parse(yon)		
		local text = handler.root.program_o.chat.line.response 
		...
		text = str .. '\n #چگونه_مردان_را_جذب_کنیم \n'
		return tdcli.sendMessage(msg.chat_id_, 0, 1, text, 1, 'html')
	end
--------------------------------
	if (matches[1]:lower() == 'love' and not Clang) or (matches[1]:lower() == 'عاشقانه' and Clang) then
		local yon = http.request('https://fatemehalzahra.com/bot/chatbot/conversation_start.php?bot_id=1&say=عاشقانه&convo_id=group_id' .. msg.to.id .. '&format=xml')	
		local handler = require("xmlhandler.tree")
		local parser = xml2lua.parser(handler)
		parser:parse(yon)		
		local text = handler.root.program_o.chat.line.response 
...
--------------------------------
	if (matches[1]:lower() == 'ali') or (matches[1]:lower() == 'علی') and matches[2] then
		local yon = http.request('https://fatemehalzahra.com/bot/chatbot/conversation_start.php?bot_id=2&say=' .. matches[2] ..'&convo_id=group_id' .. msg.to.id .. '&format=xml')	
		local handler = require("xmlhandler.tree")
		local parser = xml2lua.parser(handler)
		parser:parse(yon)		
		local text = handler.root.program_o.chat.line.response 
...
--------------------------------
	if (matches[1]:lower() == 'coins' and not Clang) or (matches[1]:lower() == 'قیمت' and Clang) then
		local coin = http.request('http://golden3.ir/bot/trade.php?t=3')
		local text = coin
		return tdcli.sendMessage(msg.chat_id_, 0, 1, text, 1, 'html')
	end
--------------------------------
