...
---------------Lock join_req-------------------
local function lock_join_req(msg, data, target) 
local hash = "gp_lang:"..msg.to.id
local lang = redis:get(hash)
if not is_mod(msg) then
if not lang then
...
local function unlock_join_req(msg, data, target)
local hash = "gp_lang:"..msg.to.id
local lang = redis:get(hash)
 if not is_mod(msg) then
if not lang then
...
--[[
---------------group channel-------------------
local function set_group_channel(msg, data, target, name) 
local hash = "gp_lang:"..msg.to.id
local lang = redis:get(hash)
if not is_mod(msg) then
if not lang then
...
local group_channel = data[tostring(target)]["settings"]["group_channel"] 
if group_channel ~= "arcaneteam" then
if not lang then
 return "*Group Channel* _Is Already added"
elseif lang then
 return "نام کانال اضافه شده است"
end
else
 data[tostring(target)]["settings"]["group_channel"] = name
save_data(_config.moderation.data, data) 
if not lang then
 return "*Group Channel* _Has Been Added"
else
 return "کانال اضافه شد!"
end
end
end

local function del_group_channel(msg, data, target)
local hash = "gp_lang:"..msg.to.id
local lang = redis:get(hash)
 if not is_mod(msg) then
if not lang then
...
local group_channel = data[tostring(target)]["settings"]["group_channel"]
 if group_channel == "arcaneteam" then
if not lang then
return "*Group Channel* _now not added!_" 
elseif lang then
return "نام کانال هم اکنون نیز ثبت نشده است!"
end
else 
data[tostring(target)]["settings"]["group_channel"] = "arcaneteam"
save_data(_config.moderation.data, data) 
if not lang then
return "*Group Channel* _Has Been Deleted!_" 
else
return "کانال حذف شد"
end
end
...
data[tostring(target)]["settings"]["welcome"] = "no"		
 ...
 if data[tostring(target)]["settings"] then		
 if not data[tostring(target)]["settings"]["join_req"] then			
 data[tostring(target)]["settings"]["join_req"] = "no"		
 end
 end
...
if ((matches[2] == "sticker" and not Clang) or (matches[2] == "برچسب" and Clang))  or (matches[2] == "استیکر" and Clang) then
...
if ((matches[1] == 'newlink' and not Clang) or (matches[1] == "لینک جدید" and Clang)) and is_mod(msg) then
 ...
 tdcli.exportChatInviteLink(msg.to.id, callback_link, nil)
  ...
  --------- channel ---------
    if ((matches[1] == "setchannel" and not Clang) or (matches[1] == "تنظیم کانال" and Clang)) and matches[2] and is_mod(msg) then
		if string.match(matches[2], "@") then
			if not lang then
				return "Please Send Channel name without *@*"
			else 
				return "لطفا نام کانال را بدون پیشوند ات ساین (@) وارد کنید!"
			end
		else
			--return set_group_channel(msg, data, target, matches[2]) 
--local group_channel = data[tostring(target)]["settings"]["group_channel"] 			
			data[tostring(chat)]['settings']["group_channel"]  = matches[2]
			save_data(_config.moderation.data, data)
			if not lang then
				return "*Group Channel* _Has Been Added"
			else 
				return "نام کانال اضافه شد!"
			end
		end
	end
  
   if ((matches[1] == "delchannel" and not Clang) or (matches[1] == "حذف کانال" and Clang)) and not matches[2] and is_mod(msg) then
    --return del_group_channel(msg, data, target)
	data[tostring(chat)]['settings']["group_channel"]  = "arcaneteam"
			save_data(_config.moderation.data, data)
			if not lang then
				return "*Group Channel* _Has Been Deleted"
			else 
				return "نام کانال حذف شد!"
			end
   end
  ------------------
...
*پاکسازی [عدد]*
_پاکسازی پیام های گروه به تعداد عدد مشخص (عدد باید بین 1 تا 1000 باشد)_
...
*اخطار* `[یوزرنیم|ایدی|ریپلی]` 
_اخطار دادن به کاربر_

*حذف اخطار* `[یوزرنیم|ایدی|ریپلی]` 
_حذف اخطار_
