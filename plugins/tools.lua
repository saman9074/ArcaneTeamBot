--Begin Tools.lua :)
local SUDO = 122630959 -- put Your ID here! <===
...
if is_sudo(msg) then
		...
		if ((matches[1]:lower() == 'support') or (matches[1] == "پشتیبانی")) and matches[2] then
		if string.match(matches[2], '^-%d+$') then
			if lang then
				tdcli.sendMessage(msg.to.id, msg.id_, 1, 'با موفقیت تورو به گروه '..matches[2]..' اضافه کردم.', 1, 'md')
				tdcli.addChatMember(matches[2], msg.from.id, 0, dl_cb, nil)
				tdcli.sendMessage(matches[2], 0, 1, '_سودو به گروه اضافه شد._', 1, 'md')
			else
				tdcli.sendMessage(msg.to.id, msg.id_, 1, '*I added you to this group:*\n\n`'..matches[2]..'`', 1, 'md')
				tdcli.addChatMember(matches[2], msg.from.id, 0, dl_cb, nil)
				tdcli.sendMessage(matches[2], 0, 1, 'Admin Joined!', 1, 'md')
			end
		end
...
local text = matches[2]
tdcli.createNewChannelChat(text, 1, '@ArcaneTeam', (function(b, d) tdcli.addChatMember(d.id_, msg.from.id, 0, dl_cb, nil) end), nil)
   if not lang then 
...
_Sudoer And Admins ArcaneTeam Bot Help :_
...
text = [[
_راهنمای ادمین و سودو های ربات آرکین:_
