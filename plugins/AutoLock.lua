...
tdcli.sendMessage(msg.chat_id_, msg.id_, 1, 'زمان بندی قبلی از سیستم حذف شد.\n\nلطفا زمان شروع قفل خودکار را وارد کنید :', 1, 'md')
redis:del("atolct1"..msg.chat_id_)
redis:del("atolct2"..msg.chat_id_)
redis:del("lc_ato:"..msg.chat_id_)
else
tdcli.sendMessage(msg.chat_id_, msg.id_, 1, 'لطفا زمان شروع قفل خودکار را وارد کنید :', 1, 'md')
end


elseif matches[1]:lower() == 'unlock auto' or matches[1] == 'باز کردن خودکار' and is_mod(msg) then
if redis:get("atolct1"..msg.chat_id_) and redis:get("atolct2"..msg.chat_id_) then
...
tdcli.sendMessage(msg.chat_id_, msg.id_, 1, 'قفل خودکار ثبت نشده است.', 1, 'md')
end

elseif matches[1]:lower() == 'lockauto stats' or matches[1] == 'وضعیت قفل خودکار' and is_mod(msg) then
...
if lc then
stats = "قفل خودکار فعال است."
else
stats = "قفل خودکار غیرفعال است."
end

tdcli.sendMessage(msg.chat_id_, msg.id_, 1, 'گروه شما در ساعات *'..t1..'* الی *'..t2..'* تعطیل خواهد شد.\n\nوضعیت فعلی گروه : '..stats, 1, 'md')
else
tdcli.sendMessage(msg.chat_id_, msg.id_, 1, 'هیچ زمانی برای تعطیلی گروه ثبت نشده است.', 1, 'md')
end
...
tdcli.sendMessage(msg.chat_id_, msg.id_, 1, 'با موفقیت انجام شد.\n\nگروه شما در ساعات *'..time_1..'* الی *'..matches[1]..':'..matches[2]..'* بصورت خودکار تعطیل خواهد شد.', 1, 'md')
...
elseif matches[1]:lower() == 'settime tehran' or matches[1]:lower() == 'تنظیم زمان تهران' and is_sudo(msg) then
io.popen("cp /usr/share/zoneinfo/Asia/Tehran /etc/localtime")
tdcli.sendMessage(msg.chat_id_, msg.id_, 1, 'زمان سرور به تهران تنظیم شد.', 1, 'md')


elseif matches[1]:lower() == 'server time' or matches[1]== 'زمان سرور' and is_sudo(msg) then
tdcli.sendMessage(msg.chat_id_, msg.id_, 1, 'ساعت ثبت شده در سرور ['..os.date("%H:%M:%S")..'] میباشد.', 1, 'md')
end
end
